...
tools/absgenftml.py -q -t 'AllChars (auto)'                      source/masters/Lateef-Regular.ufo  tests/AllChars-auto.ftml        -l logs/AllChars.log         --prevfont "$prevfont" -s "url(../$prevfont)|$prevver" --ap "_?dia[AB]$" --xsl ../tools/ftml.xsl --scale 250 -i source/glyph_data.csv --langs "sd,ur,ku,rhg,ky,wo" -w 75% -s 'url(../references/Lateef-Regular.ttf)|ref' -s 'url(../results/Lateef-ExtraLight.ttf)|ExLt' -s 'url(../results/Lateef-Regular.ttf)|Reg' -s 'url(../results/Lateef-ExtraBold.ttf)|ExBld' &
